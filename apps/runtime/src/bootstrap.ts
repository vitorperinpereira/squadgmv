...
export async function createRuntimeContext(options?: {
  ...
  const reportingService = new ReportingService(repository, queue);
  ...
  await queue.register<{
    ...
    consumersEnabled
      ? async (job) => {
          ...
          try {
            await reportingService.generateExecutiveReportSnapshot({
            ...
            logger.warn(
              { err: error, queueJobId: job.id },
              "Executive report generation did not complete."
            );
            throw error;
          }
        }
      : null,
    {
      ...
      retryBackoff: true,
      retryDelayMax: 120,
      deadLetter: "report-executive-generate-dead-letter"
    }
  );
