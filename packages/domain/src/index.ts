...
export interface RuntimeRepository {
  ...
  listExecutiveReportSnapshots(filter?: {
    limit?: number;
  }): Promise<ExecutiveReportSnapshot[]>;
...
export class ReportingService {
  ...
  async getExecutiveReport(): Promise<ExecutiveReport> {
    ...
    const missionById = new Map(missions.map((mission) => [mission.id, mission]));
    const planningItemById = new Map(
      planningItems.map((item) => [item.id, item])
    );
    ...
    >((acc, item) => {
      ...
      bucket.planningItems += 1;
      bucket.links.push(
        createPlanningItemReference(item, missionById.get(item.missionId))
      );
    ...
    const openValidationItems = planningItems.filter((item) => {
    ...
    const highlights = [
      ...
      `Itens de planejamento: ${planningItems.length} com ${pendingHandoffs.length} handoffs pendentes e ${pendingApprovals.length} approvals abertos.`,
      `Workflow history: ${workflowRuns.length} runs, ${runningWorkflowRuns.length} em andamento e ${failedWorkflowRuns.length} com falha ou compensacao.`
    ];
    const risks: string[] = [];

    if (pendingApprovals.length > 0) {
      risks.push(
        `Existem ${pendingApprovals.length} approvals pendentes de decisao.`
      );
    }

    if (openValidationItems.length > 0) {
      risks.push(
        `Existem ${openValidationItems.length} itens aguardando validacao conclusiva.`
      );
    }

    if (failedWorkflowRuns.length > 0) {
      risks.push(
        `Existem ${failedWorkflowRuns.length} workflow runs com falha ou compensacao.`
      );
    }
    ...
    for (const approval of pendingApprovals) {
      const planningItem = planningItemById.get(approval.planningItemId);
    ...
    return executiveReportSchema.parse({
      ...
      metrics: {
        ...
        validationsOpen: openValidationItems.length,
        workflowsRunning: runningWorkflowRuns.length,
        workflowsFailed: failedWorkflowRuns.length,
        memoryRecordsTotal: memoryRecords.length
      },
      metricLinks: {
        missions: uniqueReportReferences(
          missions.map((mission) => createMissionReference(mission))
        ),
        planningItems: uniqueReportReferences(
          planningItems.map((item) =>
            createPlanningItemReference(item, missionById.get(item.missionId))
          )
        ),
        handoffsPending: uniqueReportReferences(
          pendingHandoffs.map((handoff) => {
            const planningItem = planningItemById.get(handoff.taskId);
            return createHandoffReference(
              handoff,
              planningItem,
              planningItem ? missionById.get(planningItem.missionId) : undefined
            );
          })
        ),
        approvalsPending: uniqueReportReferences(
        ...
        validationsOpen: uniqueReportReferences(
          openValidationItems.map((item) =>
            createValidationReference(item, missionById.get(item.missionId))
          )
        ),
        workflowsRunning: uniqueReportReferences(
          runningWorkflowRuns.map((run) => {
            const planningItem = run.planningItemId
              ? planningItemById.get(run.planningItemId)
              : undefined;
            return createWorkflowReference(
              run,
              planningItem,
              planningItem ? missionById.get(planningItem.missionId) : undefined
            );
        ...
        workflowsFailed: uniqueReportReferences(
      ...
      optimization: {
        ...
        reviewing: optimizationInitiatives.filter(
      ...
      bySector: Object.fromEntries(
        Object.entries(bySector).map(([sector, bucket]) => [
          sector,
          {
            ...bucket,
            links: uniqueReportReferences(bucket.links)
          }
        ])
      ),
      byProcessType: Object.fromEntries(
        Object.entries(byProcessType).map(([processType, bucket]) => [
          processType,
          {
            ...bucket,
            links: uniqueReportReferences(bucket.links)
  ...
  async generateExecutiveReportSnapshot(input?: {
    trigger?: ExecutiveReportSnapshot["trigger"];
    reason?: string;
    correlationId?: string;
  }): Promise<ExecutiveReportSnapshot> {
    const report = await this.getExecutiveReport();
    const snapshot = executiveReportSnapshotSchema.parse({
      id: randomUUID(),
    ...
    await this.repository.saveExecutiveReportSnapshot(snapshot);
    await this.repository.appendAuditEvent(
      createAuditEvent(
        "executive_report",
        snapshot.id,
        ...
        null,
        input?.correlationId ?? createCorrelationId("report"),
        {
          trigger: snapshot.trigger,
          reason: snapshot.reason,
          missionsTotal: report.metrics.missionsTotal,
          planningItemsTotal: report.metrics.planningItemsTotal
        }
      )
    );
  ...
  async listExecutiveReportSnapshots(limit?: number): Promise<ExecutiveReportSnapshot[]> {
    return this.repository.listExecutiveReportSnapshots({
      limit
    });
  }

  async enqueueExecutiveReport(input?: {
    trigger?: ExecutiveReportSnapshot["trigger"];
    reason?: string;
  }): Promise<QueuePublishResult & { correlationId: string }> {
    if (!this.queue) {
      throw new Error("Executive report queue is not configured in this runtime.");
    }

    const correlationId = createCorrelationId("executive_report");
    const queueJob = await this.queue.publish("report.executive.generate", {
      trigger: input?.trigger ?? "manual",
      reason: input?.reason ?? "",
      correlationId
    });
...
function createMissionReference(
  mission: Mission
): ExecutiveReport["metricLinks"]["missions"][number] {
  return {
    entityType: "mission",
    id: mission.id,
    title: mission.title,
    ...
    planningItemId: null,
    planningItemTitle: null,
    kind: null,
    sector: "operations",
    processType: mission.processType,
    status: mission.status,
    externalUrl: mission.notionProjectUrl ?? null,
    notionPageId: mission.notionProjectPageId ?? null
  };
}

function createPlanningItemReference(
  item: PlanningItem,
  mission?: Mission
): ExecutiveReport["bySector"][string]["links"][number] {
  return {
    ...
    planningItemId: item.id,
    planningItemTitle: item.title,
    kind: item.kind,
    sector: item.sector,
    processType: item.processType,
    status: item.executionStatus,
    externalUrl: item.externalUrl ?? mission?.notionProjectUrl ?? null,
    notionPageId: item.notionPageId ?? mission?.notionProjectPageId ?? null
  };
}

function createHandoffReference(
  handoff: Handoff,
  item?: PlanningItem,
  mission?: Mission
): ExecutiveReport["metricLinks"]["handoffsPending"][number] {
  return {
    entityType: "handoff",
    id: handoff.id,
    title: handoff.taskType,
    missionId: item?.missionId ?? mission?.id ?? null,
    missionTitle: mission?.title ?? null,
    planningItemId: item?.id ?? handoff.taskId,
    planningItemTitle: item?.title ?? null,
    kind: item?.kind ?? "task",
...
function createApprovalReference(
  ...
  item?: PlanningItem,
  mission?: Mission
): ExecutiveReport["metricLinks"]["approvalsPending"][number] {
  return {
    entityType: "approval",
    id: approval.id,
...
function createValidationReference(
  item: PlanningItem,
  mission?: Mission
): ExecutiveReport["metricLinks"]["validationsOpen"][number] {
  return {
    entityType: "validation",
    id: `validation-gap:${item.id}`,
    title: item.title,
    missionId: item.missionId,
    missionTitle: mission?.title ?? null,
    planningItemId: item.id,
...
function createWorkflowReference(
  run: WorkflowRun,
  item?: PlanningItem,
  mission?: Mission
): ExecutiveReport["metricLinks"]["workflowsRunning"][number] {
  return {
    entityType: "workflow",
    id: run.id,
    title: run.jobName,
    missionId: item?.missionId ?? mission?.id ?? null,
    missionTitle: mission?.title ?? null,
    planningItemId: item?.id ?? run.planningItemId ?? null,
    planningItemTitle: item?.title ?? null,
    kind: item?.kind ?? null,
    sector: item?.sector ?? null,
    processType: item?.processType ?? null,
...
function uniqueReportReferences<T extends ExecutiveReportReference>(
  references: T[]
): T[] {
